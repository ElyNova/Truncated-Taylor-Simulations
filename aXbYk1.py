...
#The -iX gate can be done by a controlled u3 gate qc.cu3(pi, pi/2, pi, qr[a], qr[q])
#The -iZ gate can be done by a controlled rotation around the z-axis. qc.crz(pi, qr[a], qr[q])
...
qc.crz(math.pi, cont[0], psi[0])
qc.reset(cont[0])
qc.cx(anci[0], anci[1])
qc.ccx(anci[0], anci[1], cont[0])
qc.cu3(math.pi, math.pi/2, math.pi, cont[0], psi[0])
qc.reset(cont[0])
qc.cx(anci[0], anci[1])
